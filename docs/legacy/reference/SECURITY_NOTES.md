...
## 6. Logging & storage
- `soulbrowser-output/tenants/<tenant>/tasks/*.json` contains prompts, plan metadata, and LLM provider
  names. Restrict file permissions in shared servers (`chmod 700` on Linux, NTFS ACLs on Windows).
- Execution bundles (`plans.json`, `executions.json`, telemetry snapshots) now live under
  `soulbrowser-output/tenants/<tenant>/executions/<task_id>/`. Apply the same access controls because
  screenshots, logs, and telemetry may leak sensitive data.
