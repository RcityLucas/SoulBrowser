...
impl ServeState {
    ...
    pub fn new(
        ...
        Self {
            ws_url,
            config,
            perception_service: Arc::new(SyncRwLock::new(perception_service)),
            llm_cache,
            rate_limiter,
            app_context,
            health,
    ...
    pub fn execution_output_root(&self) -> PathBuf {
        self.tenant_storage_root.join("executions")
    }
    ...
    pub async fn refresh_app_context(&self) -> Result<()> {
        let context = self.build_context().await?;
        {
            ...
            *guard = Arc::clone(&context);
        }
        let new_service = Arc::new(PerceptionService::with_app_context(&context));
        *self.perception_service.write() = new_service;
        Ok(())
    }
    ...
    pub fn health_snapshot(&self) -> HealthSnapshot {
        ...
        let service = self.perception_service();
        HealthSnapshot {
            pooling_enabled: service.pooling_enabled(),
            pooling_cooldown_secs: service.pooling_cooldown_secs(),
    ...
    pub fn perception_service(&self) -> Arc<PerceptionService> {
        Arc::clone(&self.perception_service.read())
    }
}
