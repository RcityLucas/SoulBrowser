use crate::{build_exec_route, collect_events, ensure_real_chrome_enabled, wait_for_page_ready};
...
impl PerceptionService {
    ...
    async fn run_shared_perception(
        ...
        let result = execute_perception(adapter.clone(), &mut rx, &job, "shared session").await;
        drop(permit);
